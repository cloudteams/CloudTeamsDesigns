<aside class="dashboard-side-menu dashboard-side-menu-extended col-lg-3">
		<header>
			<div class="vertical-align">
				<div class="middle">
					<h1 class="header-small">Dashboard</h1>
					<p class="page-subtitle">&lt; Fold back</p>
				</div>
			</div>
		</header>

		<div class="member">
			<div class="row">
				<div class="vertical-align">
						<div class="profile-picture col-xs-4 middle">
							<img src="[[../img/profile-picture.jpg]]" class="img-circle" alt="Profile picture">
						</div>

						<div class="information col-xs-8 middle">
							<ul>
								<li class="meta big">Bobbyd123</li>
								<li class="meta">Member</li>
								<li><a href="#nowhere">Edit profile</a></li>
							</ul>
						</div>
				</div>
			</div>
		</div>

		<nav class="side-menu">
			<ul>
				<li class="projects">
					<a href="developer-dashboard-projects.php">
						<img class="icon menu-icon svg" src="[[../img/icons/icon-projects.svg]]">
						Projects
					</a>

					<ul class="side-menu-extended">
						<li class="select-dropdown">
							<i class="fa fa-angle-down"></i>
							<select class="projects-dropdown">
								<option value="Project 1" selected>Project 1</option>
								<option value="Project 1">Project 2</option>
								<option value="Project 1">Project 3</option>
								<option value="Project 1">Project 4</option>
								<option value="Project 1">Project 5</option>
							</select>
						</li>

						<li>
							<a href="developer-dashboard-project-overview.php">
								<img class="icon menu-icon svg" src="[[../img/icons/icon-seen.svg]]">
								Overview
							</a>
						</li>

						<li>
							<a href="developer-dashboard-project-services.php">
								<img class="icon menu-icon svg" src="[[../img/icons/icon-activities.svg]]">
								External services
							</a>
						</li>

						<li>
							<a href="developer-dashboard-project-profile.php">
								<img class="icon menu-icon svg" src="[[../img/icons/icon-project-profile.svg]]">
								Project profile
							</a>
						</li>

						<li>
							<a href="developer-dashboard-project-campaigns.php">
								<img class="icon menu-icon svg" src="[[../img/icons/icon-campaigns.svg]]">
								Campaigns
							</a>
						</li>

						<li>
							<a href="developer-dashboard-project-team.php">
								<img class="icon menu-icon svg" src="[[../img/icons/icon-team.svg]]">
								Team
							</a>
						</li>

						<li>
							<a href="developer-dashboard-project-documents.php">
								<img class="icon menu-icon svg" src="[[../img/icons/icon-documents.svg]]">
								Documents
							</a>
						</li>

						<li>
							<a href="developer-dashboard-project-blog.php">
								<img class="icon menu-icon svg" src="[[../img/icons/icon-blog.svg]]">
								Blog
							</a>
						</li>

						<li>
							<a href="developer-dashboard-project-messaging.php">
								<img class="icon menu-icon svg" src="[[../img/icons/icon-messaging.svg]]">
								Messaging
							</a>
						</li>

						<li>
							<a href="developer-dashboard-project-customer-ideas.php">
								<img class="icon menu-icon svg" src="[[../img/icons/icon-idea.svg]]">
								Customer ideas
							</a>
						</li>
					</ul>
				</li>

				<li>
					<a href="developer-dashboard-pending-invites.php">
						<img class="icon menu-icon svg" src="[[../img/icons/icon-pending.svg]]">
						Pending invites
					</a>
				</li>

				<li>
					<a href="developer-dashboard-activities.php">
						<img class="icon menu-icon svg" src="[[../img/icons/icon-activities.svg]]">
						Activities
					</a>
				</li>

				<li>
					<a href="developer-dashboard-notifications.php">
						<img class="icon menu-icon svg" src="[[../img/icons/icon-notifications.svg]]">
						Notifications
					</a>
				</li>
			</ul>
		</nav>
</aside>